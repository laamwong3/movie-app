...
const Tv: NextPage = () => {
  ...
  const { data: tvData, isValidating } = useSWR<Tv>(
    `/api/tv/?genre_id=${genreId}&page=${page}`,
    fetcher
  );
  const { data: genreData } = useSWR<Genre>(
    `/api/media?media_type=${"tv"}`,
    fetcher
  );
  ...
  return (
    <>
      ...
      <Grid container gap={8} justifyContent="center" marginTop={5}>
        {tvData?.results &&
          tvData.results.map((movie, index) => (
